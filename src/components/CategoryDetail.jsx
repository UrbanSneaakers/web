...
const CategoryDetail = () => {
  const { category } = useParams(); // category es navSlug
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Obtén el label del nav item para mostrar en la UI
        ...
        // Llama al RPC con navSlug
        const { data, error: rpcErr } = await supabase.rpc(
          'get_products_for_nav',
          {
            nav_slug: category,
          }
        );
  ...
  return (
    <div style={{ padding: '40px', textAlign: 'center' }}>
      <h2>Tenis de {navLabel}</h2>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center' }}>
        {sneakers.map((s) => (
          <SneaakerCard key={s.id} sneaker={s} />
        ))}
      </div>
      {sneakers.length === 0 && <p>No hay sneakers disponibles para esta categoría.</p>}
    </div>
  );
};
